...
export class UpdateAlbTagsResourceAction extends GenericResourceTaggingAction implements IResourceAction<Alb> {
  ...
  override async handle(diff: Diff<Alb, DiffValueTypeTagUpdate>): Promise<AlbSchema['response']> {
    // Get properties.
    const alb = diff.node;
  ...
  async mock(diff: Diff<Alb, DiffValueTypeTagUpdate>): Promise<AlbSchema['response']> {
    const alb = diff.node;
    return alb.response;
  }
}
