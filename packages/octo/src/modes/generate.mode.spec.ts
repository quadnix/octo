...
describe('generate()', () => {
  ...
  it('should write one folder per module with cross-module wiring', async () => {
    ...
    const resourceDiffs = await generate({ outputDir: testModes.outputDir, transaction });
  ...
  it('should preserve unrelated folders and state on regenerate', async () => {
    const { app } = await testModes.createResourceGraph();
    await generate({ outputDir: testModes.outputDir, transaction: await applyModelTransaction(app) });
