...
export function MemberSignatureBody({ hideSources, sig }: MemberSignatureBodyProps): ReactElement {
  ...
  return (
    <>
      ...
      {hasComment(sig.comment) && (showTypes || showParams || showReturn) && <hr className="tsd-divider" />}
            ...
            {sig.parameters?.map((param) => (
              <Fragment key={param.id}>
                ...
                {param.type?.type === 'union' &&
                  (param.type as unknown as Models.UnionType).types
                    .filter((unionType) => unionType.type === 'reflection')
