...
function createVersionMetadata({
  ...
  const versionLabel = (versionOptions.label ?? versionName === CURRENT_VERSION_NAME) ? 'Next' : versionName;
  let versionPathPart = (versionOptions.path ?? versionName === CURRENT_VERSION_NAME) ? 'next' : versionName;

  if (isLast) {
    versionPathPart = '';
  }

  const versionPath = normalizeUrl([context.siteConfig.baseUrl, options.routeBasePath ?? 'api', versionPathPart]);
...
export async function readVersionsMetadata(
  ...
  const specialOptions = options as DocusaurusNativePluginOptions & DocusaurusPluginTypeDocApiOptions;
  ...
  const versionNames = filterVersions(versionNamesUnfiltered, {
    ...specialOptions,
    onlyIncludeVersions:
      options.onlyIncludeVersions && options.onlyIncludeVersions.length > 0 ? options.onlyIncludeVersions : undefined,
  });
  const lastVersionName = options.lastVersion ? options.lastVersion : getDefaultLastVersionName(versionNames);
