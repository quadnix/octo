  ...
  items.forEach((item) => {
    ...
    for (const category of Object.values(referenceCategories)) {
      if (category.children.length > 0) {
        if (item.groups) {
          const index = item.groups.findIndex((g) => g.title === category.title);
          if (index === -1) {
            item.groups.push(category);
          }
        } else {
          if (!item.categories) {
            item.categories = [];
...
function mergeReflections(base: TSDDeclarationReflection, next: TSDDeclarationReflection): void {
...
function sortReflectionGroups(reflections: TSDDeclarationReflection[]): void {
  reflections.forEach((reflection) => {
    const map = createReflectionMap(reflection.children);
    const sort = (a: number, b: number): number => (map[a].name < map[b].name ? -1 : 1);
