...
and we automatically populate the availability zones because we restricted one shard per region.<br/>
...
Here, regionId can be same as `awsRegionId` because of our restriction.
- We also marked our model with the [@Model decorator](/api/octo/function/Model).

```typescript title="simple-region.model.ts"
@Model<SimpleRegion>('@example', 'region', SimpleRegionSchema)
...
:::info
Note, that unlike resources, models have fewer actions.

Models don't require a **DELETE** action because deletion is covered by user simply deleting the module they don't want
to use.

In rare cases, models might require an **UPDATE** action, but most often than not, update is also covered by user simply
updating the module inputs.
:::
