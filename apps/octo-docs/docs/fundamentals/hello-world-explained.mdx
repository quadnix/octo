...
## Libraries
The Octo ecosystem consists of
[many smaller sets of libraries](/docs/getting-started/project-structure#types-of-library) -
...
- **octo-build**: This is the CLI tool you used to run your YAML definition file.
...
and uploads the static website files as pointed by the `directoryPath` input. 
...
:::tip
As a new user and a consumer of these modules, you do not need to worry about how they create the infrastructure.
...
As you gradually learn more about how to create your own models and resources,
you will get comfortable to read any module's source code to understand the underlying infrastructure better.
:::

## Running the example
The hello-world example was run using the CLI tool.
However, the CLI tool is a just a thin wrapper around TypeScript.
Behind the scenes we generate the same TypeScript script you would have written to run the app.
So let's look at how a TypeScript script runs the app.

To run an Octo app, you first need to create a lock and initialize Octo using below code.
...
We also passed it the `HtmlReportEventListener` and `LoggingEventListener` listeners
which provides logging support and HTML reports.
...
That's why you had to pass the `settings` option to your definition file.
```yaml title="octo.yaml
settings:
  listeners:
...
We pick up the modules in your definition file, and then order them in the same order as their definition.
```typescript title="main.ts"
const moduleDefinitions = new GetModuleDefinitionsFromYAML();
...
Knowing that `app` is our root node, we get this node from the first module you defined,
and pass it to the `beginTransaction()` method,
...
It would then run each affected resource's actions, and commit the transaction by updating the state.
Finally, the app-lock is released.
```typescript title="main.ts"
try {
...
As you make changes to your `octo.yaml` definition file, either to delete the S3 bucket,
or to change one of the static website files in the `website` directory,
you will notice the new reports indicates the respective changes.
