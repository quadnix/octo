...
## Dependency Class
A dependency is not stored as a property in nodes; instead, it is its own class as a first class citizen.
...
Octo keeps track of all dependencies in a graph in the state files, and are diffed between runs.
