...
Here we are configuring it to generate the new models and the new resources that will be created.
...
await generator.next();
```
Next, we call the `next()` method on the generator to get the new models and resources.
We call `next()` one final time to let the generator finish its work.
