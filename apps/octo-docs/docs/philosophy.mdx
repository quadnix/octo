...
Octo prefers monorepo infrastructure repository by design.
...
## Polyrepo in Octo
Polyrepo is not a first class citizen in Octo, neither is it our main focus.<br/>
However, you can still have a polyrepo setup if you really want to.

You are looking to separate your infrastructure into multiple repositories.
A repository that depends on the infrastructure created by another repository
can **capture** previous repository's outputs.
This allows the dependent repository to still contain all the definitions of the entire infrastructure,
but won't create or manage them. Instead, it would simply assume those resources already exist.
```mermaid
graph TD;
  app1((App))
  app2((App))
  ...
  app2 -. capture .-> app1
  regionA2 -. capture .-> regionA1
    
  subgraph Region Repository
    app1 --> regionA1
    app1 --> regionB1
  end
  
  subgraph Envrironment Repository
    app2 --> regionA2
    regionA2 --> environmentA
  end
```

Here `Region Repository` contains the definition to create and manage App and Regions.
The `Environment Repository` captures the resources from the previous repository to define the models,
and creates an environment on top.
At runtime, the `Environment Repository` will skip processing the App and Region models,
and will only create and manage the Environment model.
...
- Are you looking to set up your infrastructure in a cloud provider other than AWS?
  ...
  - Even for AWS, we have written a limited number of modules, which only covers a small subset of AWS features.
If you are not willing to write new modules for your specific use case, Octo might be a little immature for you.
We however, are open to contribute new modules on your behalf.
